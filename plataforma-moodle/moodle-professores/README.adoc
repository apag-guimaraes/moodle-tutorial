...
Este é um recurso bastante interessante aos docentes. Através da configuração de notas, é possível ao docente criar categorias com as computações específicas para gerar as notas. Dessa forma, o professor pode, por exemplo, dividir a disciplina em categorias de 1 bimestre e 2 bimestre, cada categoria com os cálculos distintos e selecionar quais tarefas vão compor as notas de cada categoria. No final da disciplina, o docente já obtém a nota. O recurso promove automatização dos cálculos das notas!!

video::aJHVu8mVZOY[youtube,width=640,height=480]
